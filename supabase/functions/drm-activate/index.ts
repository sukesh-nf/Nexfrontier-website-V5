...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!passphrase || passphrase.length < 8) {
      return new Response(
        JSON.stringify({ ok: false, message: 'A passphrase of at least 8 characters is required.' }),
        { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (new Date(tokenRow.expires_at) < new Date()) {
      ...
      return new Response(
        JSON.stringify({ ok: false, message: 'This invitation has expired. Please request access again or contact NexFrontier for a new invitation.' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }

    // Mark invitation token as used (single-use)
    await supabase
      .from('drm_access_tokens')
    ...
    if (invError || !investor) {
      return new Response(
        JSON.stringify({ ok: false, message: 'Unable to activate access.' }),
