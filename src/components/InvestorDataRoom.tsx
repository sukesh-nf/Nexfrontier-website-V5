...
export function InvestorDataRoom() {
  ...
  const loginWithSession = useCallback(async (token: string) => {
    try {
      const data = await apiCall('drm-documents', { method: 'GET', sessionToken: token }) as { ok: boolean; investor: Investor };
  ...
  const handleInvestorActivated = (inv: Investor, token: string, sessionId?: string) => {
    setInvestor(inv);
    setSessionToken(token);
    sessionStorage.setItem('drm_session_token', token);
    setView('investor');
          ...
          {mode === 'token' && (
            <form onSubmit={handleTokenActivate} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--nf-space-5)' }}>
              ...
              <div>
                ...
                <input type="password" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} style={inputStyle} placeholder="At least 8 characters" />
              ...
              <button type="submit" disabled={loading || !token || passphrase.length < 8} style={{ ...btnStyle, opacity: loading || !token || passphrase.length < 8 ? 0.4 : 1, cursor: loading || !token || passphrase.length < 8 ? 'not-allowed' : 'pointer' }}>
...
// ============================================================
function InvestorDataRoomView({ investor, sessionToken, onLogout }: { investor: Investor; sessionToken: string; onLogout: () => void }) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await apiCall('drm-documents', { method: 'GET', sessionToken }) as { ok: boolean; folders: Folder[]; documents: Document[] };
  ...
  return (
    <DataRoomShell navItems={navItems} activeTab={tab} onTabChange={(t) => setTab(t as InvestorTab)} onLogout={onLogout} mode="investor">
      {/* Access level indicator */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--nf-space-3)', marginBottom: 'var(--nf-space-6)', padding: '10px 16px', background: 'var(--nf-bg-surface-1)', border: '1px solid var(--nf-border)', borderRadius: 'var(--nf-radius-control)' }}>
        <ShieldCheck size={16} color={investor.nda_signed ? 'var(--nf-positive)' : 'var(--nf-cyan)'} />
        <span style={{ fontSize: '0.8125rem', color: 'var(--nf-text-secondary)' }}>
          Welcome, {investor.name}. Access Level {investor.access_level} ({investor.nda_signed ? 'NDA signed - full access' : 'Public documents only'})
        </span>
      </div>

      {tab === 'overview' && <InvestorOverview investor={investor} folders={folders} documents={documents} loading={loading} error={error} />}
      {tab === 'documents' && <InvestorDocuments investor={investor} folders={folders} documents={documents} loading={loading} error={error} sessionToken={sessionToken} />}
    </DataRoomShell>
  );
}
    ...
    <div>
      ...
      {showAdd && (
        <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: 'var(--nf-space-5)', padding: 'var(--nf-space-5)', background: 'var(--nf-bg-surface-1)', border: '1px solid var(--nf-border)', borderRadius: 'var(--nf-radius-panel)' }}>
          ...
          <input type="password" value={newAdmin.passphrase} onChange={(e) => setNewAdmin({ ...newAdmin, passphrase: e.target.value })} style={inputStyle} placeholder="Passphrase (min 8 characters)" />
          <div style={{ display: 'flex', gap: '8px' }}>
